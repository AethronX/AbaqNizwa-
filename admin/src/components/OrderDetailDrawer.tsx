      ...
      >
        <div className="sticky top-0 bg-white dark:bg-[#151111] border-b border-gray-100 dark:border-gray-800 p-6 flex items-center justify-between z-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={onClose} aria-label="إغلاق" className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-500 dark:text-gray-400 transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D4AF37]">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
        ...
        <div className="p-6 space-y-6 text-sm">
          ...
          <section>
            <h4 className="text-sm font-bold text-gray-500 dark:text-gray-400 mb-3">مراحل تنفيذ الطلب</h4>
            <div className="space-y-0">
              {(order.trackingTimeline || []).map((step: any, i: number) => (
                <div key={i} className="flex gap-3">
                  ...
                  <div className="pb-4">
                    <p className={`text-sm font-bold ${step.completed ? 'text-gray-800 dark:text-gray-200' : 'text-gray-500 dark:text-gray-400'}`}>{step.titleAr}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <section className="grid grid-cols-2 gap-4">
            <div>
              <h4 className="text-sm font-bold text-gray-500 dark:text-gray-400 mb-2">العميل</h4>
            ...
            <div>
              <h4 className="text-sm font-bold text-gray-500 dark:text-gray-400 mb-2">المستلم</h4>
          ...
          {/* Shipping */}
          <section>
            <h4 className="text-sm font-bold text-gray-500 dark:text-gray-400 mb-2 flex items-center gap-1"><MapPin className="w-4 h-4" /> عنوان التوصيل</h4>
            ...
            {order.shippingAddress?.notes && <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">ملاحظات: {order.shippingAddress.notes}</p>}
          ...
          {/* Items */}
          <section>
            <h4 className="text-sm font-bold text-gray-500 dark:text-gray-400 mb-2">المنتجات ({order.items?.length || 0})</h4>
              ...
              {(order.items || []).map((item: any, i: number) => (
                <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 dark:bg-[#1A1515]">
                  ...
                  <div className="flex-1 min-w-0 text-sm">
                    ...
                    <p className="text-gray-500 dark:text-gray-400">الكمية: {item.quantity}</p>
                  </div>
          ...
          <section className="border-t border-gray-100 dark:border-gray-800 pt-4">
            <h4 className="text-sm font-bold text-gray-500 dark:text-gray-400 mb-2">ملاحظات داخلية (تظهر للإدارة فقط)</h4>
            <textarea
              value={notes}
